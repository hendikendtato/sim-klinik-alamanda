...
// Create page object
$laporan_target_pasien = new laporan_target_pasien();

// Run the page
$laporan_target_pasien->run();
...
<?php
	...
	if(isset($_POST['srhDate'])){
		...
		$result = ExecuteRow("SELECT COUNT(DISTINCT(id_pelanggan)) AS total_pelanggan, id_klinik FROM penjualan WHERE id_klinik = '$cabang' AND MONTH(waktu) = '$bulan' AND YEAR(waktu) = '$tahun'");

		$target = ExecuteScalar("SELECT target FROM m_target_pasien WHERE id_cabang = '$cabang' AND MONTH(tgl_awal) = '$bulan' AND YEAR(tgl_awal) = '$tahun'");		
	}
			...
			<table class="table table-hover table-bordered" id="printTable">
				...
				<tbody>
					...
					<tr>
						...
						<td style="text-align: right; mso-number-format:\@">
							<div class="col">
								<h5><?php echo $result['total_pelanggan']; ?></h5>
							</div>
						</td>
					</tr>
					<tr>
						...
						<td style="text-align: right; mso-number-format:\@">
							<div class="col">
								<h5>
									<?php 
										$pencapaian = $result['total_pelanggan'] - $target;
					...
					<tr>
						...
						<td style="text-align: right;">
							<div class="col">
								<h5>
									<?php
										$prosentase = round(($result['total_pelanggan'] - $target) / $target * 100);
										if($prosentase <= 0){
...
<script>
		function exportTableToExcel(tableID, filename = '') {
			...
			// Specify file name
			filename = filename ? filename + '.xls' : 'Laporan Target Pasien '+ d.toDateString() +'.xls';
...
<?php
$laporan_target_pasien->terminate();
?>