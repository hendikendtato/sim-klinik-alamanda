...
// Create page object
$laporan_perhitungan_komisi_detail = new laporan_perhitungan_komisi_detail();

// Run the page
$laporan_perhitungan_komisi_detail->run();
				...
				foreach ($result as $rs) {
					...
					echo "<tr id=".$rs["id_pegawai"].">
							...
							<tr id='".$rs["id_pegawai"]."_detil' class='collapse'>
								<td class='ew-table-last-col' colspan=3>
									<div>
										<table class='table'>
												<thead>
													<tr>
												...
												<tbody>";
													$details = ExecuteRows("SELECT * FROM transaksi_komisi
														JOIN m_barang ON transaksi_komisi.id_barang = m_barang.id
														WHERE transaksi_komisi.id_pegawai = ".$rs["id_pegawai"]." AND (tgl BETWEEN '$dateFrom' AND '$dateTo') ");
...
<?php
$laporan_perhitungan_komisi_detail->terminate();
?>