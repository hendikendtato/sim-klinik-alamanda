...
// Create page object
$laporan_target_perawatan_terjual = new laporan_target_perawatan_terjual();

// Run the page
$laporan_target_perawatan_terjual->run();
	...
	if(isset($_POST['srhDate'])){
		...
		if($multi_jenis && $jenis){
			...
			$result = ExecuteRows("SELECT * FROM m_hargajual 
						...
						JOIN jenisbarang ON m_barang.jenis = jenisbarang.id WHERE ($multi_jenis) AND m_barang.tipe = 'Perawatan' AND m_hargajual.id_klinik = '$cabang'");
			$target = ExecuteScalar("SELECT target FROM m_target_perawatan WHERE ($jenis) AND MONTH(tgl_awal) = '$bulan' AND YEAR(tgl_awal) = '$tahun'");
				...
				<thead>
					<tr>
						<td colspan="4" style="text-align: center;">
							<div class="col">
								<h4>Laporan Target Perawatan Terjual</h4>
							</div>
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td colspan="4">
							<div class="col">
								<h5>Jenis : <?php foreach ($nama_jenis as $nj) {
									echo $nj['jenis'].",";
								}  ?></h5>
							</div>
						</td>
...
<script>
		function exportTableToExcel(tableID, filename = '') {
			...
			// Specify file name
			filename = filename ? filename + '.xls' : 'Laporan Target Perawatan Terjual '+ d.toDateString() +'.xls';
...
<?php
$laporan_target_perawatan_terjual->terminate();
?>