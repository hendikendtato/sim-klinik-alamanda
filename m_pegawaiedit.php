...
loadjs.ready("head", function() {
	...
	fm_pegawaiedit.validate = function() {
		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php } ?>
			<?php if ($m_pegawai_edit->status->Required) { ?>
				elm = this.getElements("x" + infix + "_status");
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $m_pegawai_edit->status->caption(), $m_pegawai_edit->status->RequiredErrorMessage)) ?>");
			<?php } ?>
	...
	fm_pegawaiedit.lists["x_status"] = <?php echo $m_pegawai_edit->status->Lookup->toClientList($m_pegawai_edit) ?>;
	fm_pegawaiedit.lists["x_status"].options = <?php echo JsonEncode($m_pegawai_edit->status->options(FALSE, TRUE)) ?>;
...
<?php } ?>
<?php if ($m_pegawai_edit->status->Visible) { // status ?>
	<div id="r_status" class="form-group row">
		<label id="elh_m_pegawai_status" class="<?php echo $m_pegawai_edit->LeftColumnClass ?>"><?php echo $m_pegawai_edit->status->caption() ?><?php echo $m_pegawai_edit->status->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $m_pegawai_edit->RightColumnClass ?>"><div <?php echo $m_pegawai_edit->status->cellAttributes() ?>>
<span id="el_m_pegawai_status">
<div id="tp_x_status" class="ew-template"><input type="radio" class="custom-control-input" data-table="m_pegawai" data-field="x_status" data-value-separator="<?php echo $m_pegawai_edit->status->displayValueSeparatorAttribute() ?>" name="x_status" id="x_status" value="{value}"<?php echo $m_pegawai_edit->status->editAttributes() ?>></div>
<div id="dsl_x_status" data-repeatcolumn="5" class="ew-item-list d-none"><div>
<?php echo $m_pegawai_edit->status->radioButtonListHtml(FALSE, "x_status") ?>
</div></div>
</span>
<?php echo $m_pegawai_edit->status->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
