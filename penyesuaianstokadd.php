...
loadjs.ready("head", function() {
	...
	// Client script
	$("[data-field=x_jumlah]").keyup(function(){var a=$(this).attr("id");console.log(a);var l="#"+a.split("_")[0];console.log(l);var e,t=l+"_stokdatabase",i=l+"_selisih",n=l+"_tipe",o=0;e=parseFloat($(l+"_jumlah").val().split(",").join("."));var s=$(t).val().split(".").join(",");if($(t).val(s),$(t).val()?o=(o=$(t).val().split(".").join("")).replace(",","."):(o=0,$(t).val("0")),$(i).val()?$(i).val().split(".").join("").replace(".",","):0,console.log(o),console.log(e),o<e){$(n).val("Masuk");var v=e-o;v=(v=v.toFixed(2)).replace(".",","),$(i).val(v)}else{$(n).val("Keluar");var d=o-e;d=(d=d.toFixed(2)).replace(".",","),$(i).val(d)}}),$("[data-field=x_id_barang]").change(function(){var a="#"+$(this).attr("id").split("_")[0],l=a+"_stokdatabase",e=a+"_id_barang",t=$("#x_tanggal").val().split("/"),i=t[2]+"-"+t[1]+"-"+t[0],n=(k=$("#x_tanggal").val().split("/"))[2]+"-"+k[1]+"-"+k[0],o=$("[data-field=x_id_klinik]").val(),s=$(e).val(),v=new Date,d=("0"+v.getDate()).slice(-2),g=("0"+(v.getMonth()+1)).slice(-2);if(i<v.getFullYear()+"-"+g+"-"+d){var r=ew.API_URL,_="getStokAkhir",c=encodeURIComponent(s),p=encodeURIComponent(o),k=n;$.get(r+"?action="+_+"&id_barang="+c+"&id_klinik="+p+"&tanggal="+k,function(a){a&&$(l).val(a)})}else{r=ew.API_URL,_="getStok",c=encodeURIComponent(s),p=encodeURIComponent(o);$.get(r+"?action="+_+"&id_barang="+c+"&id_klinik="+p,function(a){a&&$(l).val(a)})}});
});
</script>
