...
loadjs.ready("head", function() {
	...
	// Client script
	$("[data-field=x_jumlah]").keyup(function(){var a=$(this).attr("id");console.log(a);var e="#"+a.split("_")[0];console.log(e);var l,t=e+"_stokdatabase",n=e+"_selisih",i=e+"_tipe",o=0;l=parseFloat($(e+"_jumlah").val().split(",").join("."));var d=$(t).val().split(".").join(",");if($(t).val(d),$(t).val()?o=(o=$(t).val().split(".").join("")).replace(",","."):(o=0,$(t).val("0")),$(n).val()?$(n).val().split(".").join("").replace(".",","):0,console.log(o),console.log(l),o<l){$(i).val("Masuk");var g=l-o;g=(g=g.toFixed(2)).replace(".",","),$(n).val(g)}else{$(i).val("Keluar");var s=o-l;s=(s=s.toFixed(2)).replace(".",","),$(n).val(s)}}),$("[data-field=x_id_barang]").change(function(){var a="#"+$(this).attr("id").split("_")[0],e=a+"_stokdatabase",l=a+"_id_barang",t=$("#x_tanggal").val().split("/"),n=t[2]+"-"+t[1]+"-"+t[0],i=(k=$("#x_tanggal").val().split("/"))[2]+"-"+k[1]+"-"+k[0],o=$("[data-field=x_id_klinik]").val(),d=$(l).val(),g=new Date,s=("0"+g.getDate()).slice(-2),v=("0"+(g.getMonth()+1)).slice(-2);if(n<g.getFullYear()+"-"+v+"-"+s){var r=ew.API_URL,_="getStokAkhir",c=encodeURIComponent(d),p=encodeURIComponent(o),k=i;$.get(r+"?action="+_+"&id_barang="+c+"&id_klinik="+p+"&tanggal="+k,function(a){a&&$(e).val(a)})}else{r=ew.API_URL,_="getStok",c=encodeURIComponent(d),p=encodeURIComponent(o);$.get(r+"?action="+_+"&id_barang="+c+"&id_klinik="+p,function(a){a&&$(e).val(a)})}});var now=new Date,day=("0"+now.getDate()).slice(-2),month=("0"+(now.getMonth()+1)).slice(-2),today=day+"/"+month+"/"+now.getFullYear();$("input#x_tanggal").val(today),$("input#x_tanggal").prop("disabled",!0);
});
</script>
