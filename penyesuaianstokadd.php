...
loadjs.ready("head", function() {
	...
	// Client script
	$("[data-field=x_jumlah]").keyup(function(){var a=$(this).attr("id");console.log(a);var e="#"+a.split("_")[0];console.log(e);var l,t=e+"_stokdatabase",i=e+"_selisih",n=e+"_tipe",o=0;if(l=parseFloat($(e+"_jumlah").val().split(",").join(".")),$(t).val()?o=(o=$(t).val().split(".").join("")).replace(",","."):(o=0,$(t).val("0")),$(i).val()?$(i).val().split(".").join("").replace(".",","):0,console.log(o),console.log(l),o<l){$(n).val("Masuk");var s=l-o;s=(s=s.toFixed(2)).replace(".",","),$(i).val(s)}else{$(n).val("Keluar");var d=o-l;d=(d=d.toFixed(2)).replace(".",","),$(i).val(d)}}),$("[data-field=x_id_barang]").change(function(){var a="#"+$(this).attr("id").split("_")[0],e=a+"_stokdatabase",l=a+"_id_barang",t=$("#x_tanggal").val().split("/"),i=t[2]+"-"+t[1]+"-"+t[0],n=(k=$("#x_tanggal").val().split("/"))[2]+"-"+k[1]+"-"+k[0],o=$("[data-field=x_id_klinik]").val(),s=$(l).val(),d=new Date,v=("0"+d.getDate()).slice(-2),g=("0"+(d.getMonth()+1)).slice(-2);if(i<d.getFullYear()+"-"+g+"-"+v){var _=ew.API_URL,c="getStokAkhir",r=encodeURIComponent(s),p=encodeURIComponent(o),k=n;$.get(_+"?action="+c+"&id_barang="+r+"&id_klinik="+p+"&tanggal="+k,function(a){a&&$(e).val(a)})}else{_=ew.API_URL,c="getStok",r=encodeURIComponent(s),p=encodeURIComponent(o);$.get(_+"?action="+c+"&id_barang="+r+"&id_klinik="+p,function(a){a&&$(e).val(a)})}});
});
</script>
