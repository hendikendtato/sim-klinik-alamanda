...
/**
 * Table class for m_target_produk
 */
class m_target_produk extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'm_target_produk';
		$this->TableName = 'm_target_produk';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`m_target_produk`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id_target_produk
		$this->id_target_produk = new DbField('m_target_produk', 'm_target_produk', 'x_id_target_produk', 'id_target_produk', '`id_target_produk`', '`id_target_produk`', 3, 11, -1, FALSE, '`id_target_produk`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id_target_produk->IsAutoIncrement = TRUE; // Autoincrement field
		$this->id_target_produk->IsPrimaryKey = TRUE; // Primary key field
		$this->id_target_produk->Sortable = TRUE; // Allow sort
		$this->id_target_produk->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['id_target_produk'] = &$this->id_target_produk;

		// id_cabang
		$this->id_cabang = new DbField('m_target_produk', 'm_target_produk', 'x_id_cabang', 'id_cabang', '`id_cabang`', '`id_cabang`', 3, 11, -1, FALSE, '`id_cabang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// status
		$this->status = new DbField('m_target_produk', 'm_target_produk', 'x_status', 'status', '`status`', '`status`', 200, 255, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
		$this->status->Sortable = TRUE; // Allow sort
		$this->status->Lookup = new Lookup('status', 'm_status_barang', FALSE, 'id_status', ["status_barang","","",""], [], [], [], [], [], [], '', '');
		$this->fields['status'] = &$this->status;

		// tgl_awal
		$this->tgl_awal = new DbField('m_target_produk', 'm_target_produk', 'x_tgl_awal', 'tgl_awal', '`tgl_awal`', CastDateFieldForLike("`tgl_awal`", 0, "DB"), 133, 10, 0, FALSE, '`tgl_awal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// tgl_akhir
		$this->tgl_akhir = new DbField('m_target_produk', 'm_target_produk', 'x_tgl_akhir', 'tgl_akhir', '`tgl_akhir`', CastDateFieldForLike("`tgl_akhir`", 0, "DB"), 133, 10, 0, FALSE, '`tgl_akhir`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// target
		$this->target = new DbField('m_target_produk', 'm_target_produk', 'x_target', 'target', '`target`', '`target`', 5, 22, -1, FALSE, '`target`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// created
		$this->created = new DbField('m_target_produk', 'm_target_produk', 'x_created', 'created', '`created`', '`created`', 200, 255, -1, FALSE, '`created`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->created->Sortable = TRUE; // Allow sort
		$this->fields['created'] = &$this->created;

		// updated
		$this->updated = new DbField('m_target_produk', 'm_target_produk', 'x_updated', 'updated', '`updated`', '`updated`', 200, 255, -1, FALSE, '`updated`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`m_target_produk`";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->id_target_produk->setDbValue($conn->insert_ID());
			$rs['id_target_produk'] = $this->id_target_produk->DbValue;
		}
		return $success;
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('id_target_produk', $rs))
				AddFilter($where, QuotedName('id_target_produk', $this->Dbid) . '=' . QuotedValue($rs['id_target_produk'], $this->id_target_produk->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->id_target_produk->DbValue = $row['id_target_produk'];
	...
	{
		return "`id_target_produk` = @id_target_produk@";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('id_target_produk', $row) ? $row['id_target_produk'] : NULL;
		else
			$val = $this->id_target_produk->OldValue !== NULL ? $this->id_target_produk->OldValue : $this->id_target_produk->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@id_target_produk@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "m_target_produklist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "m_target_produkview.php")
			return $Language->phrase("View");
		elseif ($pageName == "m_target_produkedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "m_target_produkadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_target_produkview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_target_produkview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "m_target_produkadd.php?" . $this->getUrlParm($parm);
		else
			$url = "m_target_produkadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_target_produkedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_target_produkadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("m_target_produkdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "id_target_produk:" . JsonEncode($this->id_target_produk->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm != "")
			$url .= $parm . "&";
		if ($this->id_target_produk->CurrentValue != NULL) {
			$url .= "id_target_produk=" . urlencode($this->id_target_produk->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("id_target_produk") !== NULL)
				$arKeys[] = Param("id_target_produk");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->id_target_produk->CurrentValue = $key;
			else
				$this->id_target_produk->OldValue = $key;
	...
	{
		$this->id_target_produk->setDbValue($rs->fields('id_target_produk'));
	...
	public function renderListRow()
	{
		...
		$this->id_target_produk->HrefValue = "";
		$this->id_target_produk->TooltipValue = "";
	...
	public function renderEditRow()
	{
		...
		// id_target_produk
		$this->id_target_produk->EditAttrs["class"] = "form-control";
		$this->id_target_produk->EditCustomAttributes = "";
		$this->id_target_produk->EditValue = $this->id_target_produk->CurrentValue;
		$this->id_target_produk->ViewCustomAttributes = "";
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					$doc->exportCaption($this->id_target_produk);
		...
		while (!$recordset->EOF && $recCnt < $stopRec) {
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						$doc->exportField($this->id_target_produk);
