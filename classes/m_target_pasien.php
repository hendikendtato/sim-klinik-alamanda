...
/**
 * Table class for m_target_pasien
 */
class m_target_pasien extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'm_target_pasien';
		$this->TableName = 'm_target_pasien';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`m_target_pasien`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id_target_pasien
		$this->id_target_pasien = new DbField('m_target_pasien', 'm_target_pasien', 'x_id_target_pasien', 'id_target_pasien', '`id_target_pasien`', '`id_target_pasien`', 3, 11, -1, FALSE, '`id_target_pasien`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id_target_pasien->IsAutoIncrement = TRUE; // Autoincrement field
		$this->id_target_pasien->IsPrimaryKey = TRUE; // Primary key field
		$this->id_target_pasien->Sortable = TRUE; // Allow sort
		$this->id_target_pasien->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['id_target_pasien'] = &$this->id_target_pasien;

		// id_cabang
		$this->id_cabang = new DbField('m_target_pasien', 'm_target_pasien', 'x_id_cabang', 'id_cabang', '`id_cabang`', '`id_cabang`', 3, 11, -1, FALSE, '`id_cabang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// tgl_awal
		$this->tgl_awal = new DbField('m_target_pasien', 'm_target_pasien', 'x_tgl_awal', 'tgl_awal', '`tgl_awal`', CastDateFieldForLike("`tgl_awal`", 0, "DB"), 133, 10, 0, FALSE, '`tgl_awal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// tgl_akhir
		$this->tgl_akhir = new DbField('m_target_pasien', 'm_target_pasien', 'x_tgl_akhir', 'tgl_akhir', '`tgl_akhir`', CastDateFieldForLike("`tgl_akhir`", 0, "DB"), 133, 10, 0, FALSE, '`tgl_akhir`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`m_target_pasien`";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->id_target_pasien->setDbValue($conn->insert_ID());
			$rs['id_target_pasien'] = $this->id_target_pasien->DbValue;
		}
		return $success;
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('id_target_pasien', $rs))
				AddFilter($where, QuotedName('id_target_pasien', $this->Dbid) . '=' . QuotedValue($rs['id_target_pasien'], $this->id_target_pasien->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->id_target_pasien->DbValue = $row['id_target_pasien'];
	...
	{
		return "`id_target_pasien` = @id_target_pasien@";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('id_target_pasien', $row) ? $row['id_target_pasien'] : NULL;
		else
			$val = $this->id_target_pasien->OldValue !== NULL ? $this->id_target_pasien->OldValue : $this->id_target_pasien->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@id_target_pasien@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "m_target_pasienlist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "m_target_pasienview.php")
			return $Language->phrase("View");
		elseif ($pageName == "m_target_pasienedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "m_target_pasienadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_target_pasienview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_target_pasienview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "m_target_pasienadd.php?" . $this->getUrlParm($parm);
		else
			$url = "m_target_pasienadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_target_pasienedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_target_pasienadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("m_target_pasiendelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "id_target_pasien:" . JsonEncode($this->id_target_pasien->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm != "")
			$url .= $parm . "&";
		if ($this->id_target_pasien->CurrentValue != NULL) {
			$url .= "id_target_pasien=" . urlencode($this->id_target_pasien->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("id_target_pasien") !== NULL)
				$arKeys[] = Param("id_target_pasien");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->id_target_pasien->CurrentValue = $key;
			else
				$this->id_target_pasien->OldValue = $key;
	...
	{
		$this->id_target_pasien->setDbValue($rs->fields('id_target_pasien'));
	...
	public function renderListRow()
	{
		...
		$this->id_target_pasien->HrefValue = "";
		$this->id_target_pasien->TooltipValue = "";
	...
	public function renderEditRow()
	{
		...
		// id_target_pasien
		$this->id_target_pasien->EditAttrs["class"] = "form-control";
		$this->id_target_pasien->EditCustomAttributes = "";
		$this->id_target_pasien->EditValue = $this->id_target_pasien->CurrentValue;
		$this->id_target_pasien->ViewCustomAttributes = "";
