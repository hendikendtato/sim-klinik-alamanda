...
/**
 * Table class for laporan_rekening
 */
class laporan_rekening extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'laporan_rekening';
		$this->TableName = 'laporan_rekening';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`laporan_rekening`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('laporan_rekening', 'laporan_rekening', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// id_rekening
		$this->id_rekening = new DbField('laporan_rekening', 'laporan_rekening', 'x_id_rekening', 'id_rekening', '`id_rekening`', '`id_rekening`', 3, 11, -1, FALSE, '`id_rekening`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id_rekening->Nullable = FALSE; // NOT NULL field
		$this->id_rekening->Required = TRUE; // Required field
		...
		// id_klinik
		$this->id_klinik = new DbField('laporan_rekening', 'laporan_rekening', 'x_id_klinik', 'id_klinik', '`id_klinik`', '`id_klinik`', 3, 11, -1, FALSE, '`id_klinik`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// jumlah
		$this->jumlah = new DbField('laporan_rekening', 'laporan_rekening', 'x_jumlah', 'jumlah', '`jumlah`', '`jumlah`', 5, 22, -1, FALSE, '`jumlah`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->jumlah->Sortable = TRUE; // Allow sort
		$this->jumlah->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
		$this->fields['jumlah'] = &$this->jumlah;

		// tanggal
		$this->tanggal = new DbField('laporan_rekening', 'laporan_rekening', 'x_tanggal', 'tanggal', '`tanggal`', CastDateFieldForLike("`tanggal`", 0, "DB"), 133, 10, 0, FALSE, '`tanggal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->tanggal->Sortable = TRUE; // Allow sort
		$this->tanggal->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['tanggal'] = &$this->tanggal;

		// kode_penjualan
		$this->kode_penjualan = new DbField('laporan_rekening', 'laporan_rekening', 'x_kode_penjualan', 'kode_penjualan', '`kode_penjualan`', '`kode_penjualan`', 200, 50, -1, FALSE, '`kode_penjualan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->kode_penjualan->Sortable = TRUE; // Allow sort
		$this->fields['kode_penjualan'] = &$this->kode_penjualan;

		// saldo_awal
		$this->saldo_awal = new DbField('laporan_rekening', 'laporan_rekening', 'x_saldo_awal', 'saldo_awal', '`saldo_awal`', '`saldo_awal`', 5, 22, -1, FALSE, '`saldo_awal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// sisa_saldo
		$this->sisa_saldo = new DbField('laporan_rekening', 'laporan_rekening', 'x_sisa_saldo', 'sisa_saldo', '`sisa_saldo`', '`sisa_saldo`', 5, 22, -1, FALSE, '`sisa_saldo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`laporan_rekening`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "laporan_rekeninglist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "laporan_rekeningview.php")
			return $Language->phrase("View");
		elseif ($pageName == "laporan_rekeningedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "laporan_rekeningadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("laporan_rekeningview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("laporan_rekeningview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "laporan_rekeningadd.php?" . $this->getUrlParm($parm);
		else
			$url = "laporan_rekeningadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("laporan_rekeningedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("laporan_rekeningadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("laporan_rekeningdelete.php", $this->getUrlParm());
	}
	...
	public function renderListRow()
	{
		...
		$this->id_rekening->ViewValue = FormatNumber($this->id_rekening->ViewValue, 0, -2, -2, -2);
	...
	public function renderEditRow()
	{
		...
		$this->id_rekening->PlaceHolder = RemoveHtml($this->id_rekening->caption());
