...
/**
 * Table class for m_target_omset_cabang
 */
class m_target_omset_cabang extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'm_target_omset_cabang';
		$this->TableName = 'm_target_omset_cabang';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`m_target_omset_cabang`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		$this->id_target_omset_cabang = new DbField('m_target_omset_cabang', 'm_target_omset_cabang', 'x_id_target_omset_cabang', 'id_target_omset_cabang', '`id_target_omset_cabang`', '`id_target_omset_cabang`', 3, 11, -1, FALSE, '`id_target_omset_cabang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id_target_omset_cabang->IsAutoIncrement = TRUE; // Autoincrement field
		$this->id_target_omset_cabang->IsPrimaryKey = TRUE; // Primary key field
		$this->id_target_omset_cabang->Sortable = TRUE; // Allow sort
		$this->id_target_omset_cabang->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['id_target_omset_cabang'] = &$this->id_target_omset_cabang;

		// id_cabang
		$this->id_cabang = new DbField('m_target_omset_cabang', 'm_target_omset_cabang', 'x_id_cabang', 'id_cabang', '`id_cabang`', '`id_cabang`', 3, 11, -1, FALSE, '`id_cabang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// tgl_awal
		$this->tgl_awal = new DbField('m_target_omset_cabang', 'm_target_omset_cabang', 'x_tgl_awal', 'tgl_awal', '`tgl_awal`', CastDateFieldForLike("`tgl_awal`", 0, "DB"), 133, 10, 0, FALSE, '`tgl_awal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// tgl_akhir
		$this->tgl_akhir = new DbField('m_target_omset_cabang', 'm_target_omset_cabang', 'x_tgl_akhir', 'tgl_akhir', '`tgl_akhir`', CastDateFieldForLike("`tgl_akhir`", 0, "DB"), 133, 10, 0, FALSE, '`tgl_akhir`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// target
		$this->target = new DbField('m_target_omset_cabang', 'm_target_omset_cabang', 'x_target', 'target', '`target`', '`target`', 5, 22, -1, FALSE, '`target`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// baseline
		$this->baseline = new DbField('m_target_omset_cabang', 'm_target_omset_cabang', 'x_baseline', 'baseline', '`baseline`', '`baseline`', 5, 22, -1, FALSE, '`baseline`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// aset
		$this->aset = new DbField('m_target_omset_cabang', 'm_target_omset_cabang', 'x_aset', 'aset', '`aset`', '`aset`', 5, 22, -1, FALSE, '`aset`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`m_target_omset_cabang`";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->id_target_omset_cabang->setDbValue($conn->insert_ID());
			$rs['id_target_omset_cabang'] = $this->id_target_omset_cabang->DbValue;
		}
		return $success;
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('id_target_omset_cabang', $rs))
				AddFilter($where, QuotedName('id_target_omset_cabang', $this->Dbid) . '=' . QuotedValue($rs['id_target_omset_cabang'], $this->id_target_omset_cabang->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->id_target_omset_cabang->DbValue = $row['id_target_omset_cabang'];
	...
	{
		return "`id_target_omset_cabang` = @id_target_omset_cabang@";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('id_target_omset_cabang', $row) ? $row['id_target_omset_cabang'] : NULL;
		else
			$val = $this->id_target_omset_cabang->OldValue !== NULL ? $this->id_target_omset_cabang->OldValue : $this->id_target_omset_cabang->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@id_target_omset_cabang@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "m_target_omset_cabanglist.php";
		}
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "m_target_omset_cabangedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "m_target_omset_cabangadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_target_omset_cabangview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_target_omset_cabangview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "m_target_omset_cabangadd.php?" . $this->getUrlParm($parm);
		else
			$url = "m_target_omset_cabangadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_target_omset_cabangedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_target_omset_cabangadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("m_target_omset_cabangdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "id_target_omset_cabang:" . JsonEncode($this->id_target_omset_cabang->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm != "")
			$url .= $parm . "&";
		if ($this->id_target_omset_cabang->CurrentValue != NULL) {
			$url .= "id_target_omset_cabang=" . urlencode($this->id_target_omset_cabang->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("id_target_omset_cabang") !== NULL)
				$arKeys[] = Param("id_target_omset_cabang");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->id_target_omset_cabang->CurrentValue = $key;
			else
				$this->id_target_omset_cabang->OldValue = $key;
	...
	{
		$this->id_target_omset_cabang->setDbValue($rs->fields('id_target_omset_cabang'));
	...
	public function renderListRow()
	{
		...
		$this->id_target_omset_cabang->CellCssStyle = "white-space: nowrap;";
		...
		$this->id_target_omset_cabang->TooltipValue = "";
	...
	public function renderEditRow()
	{
		...
		// id_target_omset_cabang
		$this->id_target_omset_cabang->EditAttrs["class"] = "form-control";
		$this->id_target_omset_cabang->EditCustomAttributes = "";
		$this->id_target_omset_cabang->EditValue = $this->id_target_omset_cabang->CurrentValue;
		$this->id_target_omset_cabang->ViewCustomAttributes = "";
