...
/**
 * Table class for nonjual
 */
class nonjual extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'nonjual';
		$this->TableName = 'nonjual';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`nonjual`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id_nonjual
		$this->id_nonjual = new DbField('nonjual', 'nonjual', 'x_id_nonjual', 'id_nonjual', '`id_nonjual`', '`id_nonjual`', 3, 11, -1, FALSE, '`id_nonjual`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id_nonjual->IsAutoIncrement = TRUE; // Autoincrement field
		$this->id_nonjual->IsPrimaryKey = TRUE; // Primary key field
		...
		// id_klinik
		$this->id_klinik = new DbField('nonjual', 'nonjual', 'x_id_klinik', 'id_klinik', '`id_klinik`', '`id_klinik`', 3, 11, -1, FALSE, '`id_klinik`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// keterangan
		$this->keterangan = new DbField('nonjual', 'nonjual', 'x_keterangan', 'keterangan', '`keterangan`', '`keterangan`', 200, 255, -1, FALSE, '`keterangan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "detail_nonjual") {
			$detailUrl = $GLOBALS["detail_nonjual"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_id_nonjual=" . urlencode($this->id_nonjual->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "nonjuallist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`nonjual`";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->id_nonjual->setDbValue($conn->insert_ID());
			$rs['id_nonjual'] = $this->id_nonjual->DbValue;
		}
		return $success;
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('id_nonjual', $rs))
				AddFilter($where, QuotedName('id_nonjual', $this->Dbid) . '=' . QuotedValue($rs['id_nonjual'], $this->id_nonjual->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('id_nonjual', $row) ? $row['id_nonjual'] : NULL;
		else
			$val = $this->id_nonjual->OldValue !== NULL ? $this->id_nonjual->OldValue : $this->id_nonjual->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@id_nonjual@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "nonjualedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "nonjualadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("nonjualview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("nonjualview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "nonjualadd.php?" . $this->getUrlParm($parm);
		else
			$url = "nonjualadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("nonjualedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("nonjualedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("nonjualadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("nonjualadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("nonjualdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "id_nonjual:" . JsonEncode($this->id_nonjual->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm != "")
			$url .= $parm . "&";
		if ($this->id_nonjual->CurrentValue != NULL) {
			$url .= "id_nonjual=" . urlencode($this->id_nonjual->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("id_nonjual") !== NULL)
				$arKeys[] = Param("id_nonjual");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->id_nonjual->CurrentValue = $key;
			else
				$this->id_nonjual->OldValue = $key;
