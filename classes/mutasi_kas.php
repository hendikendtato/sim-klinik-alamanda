...
/**
 * Table class for mutasi_kas
 */
class mutasi_kas extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'mutasi_kas';
		$this->TableName = 'mutasi_kas';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`mutasi_kas`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('mutasi_kas', 'mutasi_kas', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// no_bukti
		$this->no_bukti = new DbField('mutasi_kas', 'mutasi_kas', 'x_no_bukti', 'no_bukti', '`no_bukti`', '`no_bukti`', 200, 50, -1, FALSE, '`no_bukti`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// id_klinik
		$this->id_klinik = new DbField('mutasi_kas', 'mutasi_kas', 'x_id_klinik', 'id_klinik', '`id_klinik`', '`id_klinik`', 3, 11, -1, FALSE, '`id_klinik`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->id_klinik->Lookup = new Lookup('id_klinik', 'm_klinik', FALSE, 'id_klinik', ["nama_klinik","","",""], [], ["detailmutasibank x_akun_id","detailmutasibank x_nama_akun","x_id_kas"], [], [], [], [], '', '');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "detailmutasibank") {
			$detailUrl = $GLOBALS["detailmutasibank"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_id=" . urlencode($this->id->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "mutasi_kaslist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`mutasi_kas`";
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "mutasi_kasedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "mutasi_kasadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("mutasi_kasview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("mutasi_kasview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "mutasi_kasadd.php?" . $this->getUrlParm($parm);
		else
			$url = "mutasi_kasadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("mutasi_kasedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("mutasi_kasedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("mutasi_kasadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("mutasi_kasadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("mutasi_kasdelete.php", $this->getUrlParm());
	}
	...
	public function renderEditRow()
	{
		...
		if (!$this->no_bukti->Raw)
			$this->no_bukti->CurrentValue = HtmlDecode($this->no_bukti->CurrentValue);
		$this->no_bukti->EditValue = $this->no_bukti->CurrentValue;
		$this->no_bukti->PlaceHolder = RemoveHtml($this->no_bukti->caption());
	...
	function Row_Inserting($rsold, &$rsnew) {
		...
		if($tipe == 'Mutasi Kas Masuk'){
			$kode_mutasi_sebelumnya = ExecuteScalar("SELECT no_bukti FROM mutasi_kas WHERE id_klinik='$id_klinik' AND no_bukti LIKE '%KM%'  ORDER BY id DESC LIMIT 1");
			...
			/* kode penjualan dengan format KBMxx-yymm-0000
			* KBM = Barang Masuk
			*	xx = kode cabang
			*	yy = tahun
			*	mm = bulan
			*	0000 = nmr urut
			*/
			$rsnew['no_bukti'] = 'KM' . $id_klinik . '-' . date('ym') . '-' . $nomor_urut;
		} else {
			$kode_mutasi_sebelumnya = ExecuteScalar("SELECT no_bukti FROM mutasi_kas WHERE id_klinik='$id_klinik' AND no_bukti LIKE '%KK%'  ORDER BY id DESC LIMIT 1");
			$kode = explode('-', $kode_mutasi_sebelumnya);
			$nomor_urut_terakhir = $kode[2];
			$bulan_sebelumnya = substr($kode[1], -2);
			$nomor_urut = '0000';
			...
			/* KBK = Mutasi Bank Keluar
			*	xx = kode cabang
			*	yy = tahun
			*	mm = bulan
			*	0000 = nmr urut
			*/
			$rsnew['no_bukti'] = 'KK' . $id_klinik . '-' . date('ym') . '-' . $nomor_urut;
		}
		return TRUE;
	}
