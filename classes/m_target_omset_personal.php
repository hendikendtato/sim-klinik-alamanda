...
/**
 * Table class for m_target_omset_personal
 */
class m_target_omset_personal extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	// Fields
	public $id_target_omset_personal;
	public $id_cabang;
	public $id_jabatan;
	public $tgl_awal;
	public $tgl_akhir;
	...
	public function __construct()
	{
		...
		$this->TableVar = 'm_target_omset_personal';
		$this->TableName = 'm_target_omset_personal';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`m_target_omset_personal`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		$this->id_target_omset_personal = new DbField('m_target_omset_personal', 'm_target_omset_personal', 'x_id_target_omset_personal', 'id_target_omset_personal', '`id_target_omset_personal`', '`id_target_omset_personal`', 3, 11, -1, FALSE, '`id_target_omset_personal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id_target_omset_personal->IsAutoIncrement = TRUE; // Autoincrement field
		$this->id_target_omset_personal->IsPrimaryKey = TRUE; // Primary key field
		$this->id_target_omset_personal->Sortable = TRUE; // Allow sort
		$this->id_target_omset_personal->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['id_target_omset_personal'] = &$this->id_target_omset_personal;

		// id_cabang
		$this->id_cabang = new DbField('m_target_omset_personal', 'm_target_omset_personal', 'x_id_cabang', 'id_cabang', '`id_cabang`', '`id_cabang`', 3, 11, -1, FALSE, '`id_cabang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// id_jabatan
		$this->id_jabatan = new DbField('m_target_omset_personal', 'm_target_omset_personal', 'x_id_jabatan', 'id_jabatan', '`id_jabatan`', '`id_jabatan`', 3, 11, -1, FALSE, '`id_jabatan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// tgl_awal
		$this->tgl_awal = new DbField('m_target_omset_personal', 'm_target_omset_personal', 'x_tgl_awal', 'tgl_awal', '`tgl_awal`', CastDateFieldForLike("`tgl_awal`", 0, "DB"), 133, 10, 0, FALSE, '`tgl_awal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// tgl_akhir
		$this->tgl_akhir = new DbField('m_target_omset_personal', 'm_target_omset_personal', 'x_tgl_akhir', 'tgl_akhir', '`tgl_akhir`', CastDateFieldForLike("`tgl_akhir`", 0, "DB"), 133, 10, 0, FALSE, '`tgl_akhir`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// target
		$this->target = new DbField('m_target_omset_personal', 'm_target_omset_personal', 'x_target', 'target', '`target`', '`target`', 5, 22, -1, FALSE, '`target`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`m_target_omset_personal`";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->id_target_omset_personal->setDbValue($conn->insert_ID());
			$rs['id_target_omset_personal'] = $this->id_target_omset_personal->DbValue;
		}
		return $success;
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('id_target_omset_personal', $rs))
				AddFilter($where, QuotedName('id_target_omset_personal', $this->Dbid) . '=' . QuotedValue($rs['id_target_omset_personal'], $this->id_target_omset_personal->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->id_target_omset_personal->DbValue = $row['id_target_omset_personal'];
	...
	{
		return "`id_target_omset_personal` = @id_target_omset_personal@";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('id_target_omset_personal', $row) ? $row['id_target_omset_personal'] : NULL;
		else
			$val = $this->id_target_omset_personal->OldValue !== NULL ? $this->id_target_omset_personal->OldValue : $this->id_target_omset_personal->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@id_target_omset_personal@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "m_target_omset_personallist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "m_target_omset_personalview.php")
			return $Language->phrase("View");
		elseif ($pageName == "m_target_omset_personaledit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "m_target_omset_personaladd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_target_omset_personalview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_target_omset_personalview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "m_target_omset_personaladd.php?" . $this->getUrlParm($parm);
		else
			$url = "m_target_omset_personaladd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_target_omset_personaledit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_target_omset_personaladd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("m_target_omset_personaldelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "id_target_omset_personal:" . JsonEncode($this->id_target_omset_personal->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm != "")
			$url .= $parm . "&";
		if ($this->id_target_omset_personal->CurrentValue != NULL) {
			$url .= "id_target_omset_personal=" . urlencode($this->id_target_omset_personal->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("id_target_omset_personal") !== NULL)
				$arKeys[] = Param("id_target_omset_personal");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->id_target_omset_personal->CurrentValue = $key;
			else
				$this->id_target_omset_personal->OldValue = $key;
	...
	{
		$this->id_target_omset_personal->setDbValue($rs->fields('id_target_omset_personal'));
	...
	public function renderListRow()
	{
		...
		$this->id_target_omset_personal->CellCssStyle = "white-space: nowrap;";
		...
		$this->id_target_omset_personal->ViewCustomAttributes = "";
		...
		$this->id_target_omset_personal->TooltipValue = "";
	...
	public function renderEditRow()
	{
		...
		// id_target_omset_personal
		$this->id_target_omset_personal->EditAttrs["class"] = "form-control";
		$this->id_target_omset_personal->EditCustomAttributes = "";
		$this->id_target_omset_personal->EditValue = $this->id_target_omset_personal->CurrentValue;
		$this->id_target_omset_personal->ViewCustomAttributes = "";
