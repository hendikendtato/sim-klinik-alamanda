...
/**
 * Table class for laporan_kas
 */
class laporan_kas extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'laporan_kas';
		$this->TableName = 'laporan_kas';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`laporan_kas`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('laporan_kas', 'laporan_kas', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// id_klinik
		$this->id_klinik = new DbField('laporan_kas', 'laporan_kas', 'x_id_klinik', 'id_klinik', '`id_klinik`', '`id_klinik`', 3, 11, -1, FALSE, '`id_klinik`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// id_kas
		$this->id_kas = new DbField('laporan_kas', 'laporan_kas', 'x_id_kas', 'id_kas', '`id_kas`', '`id_kas`', 3, 11, -1, FALSE, '`id_kas`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// jumlah
		$this->jumlah = new DbField('laporan_kas', 'laporan_kas', 'x_jumlah', 'jumlah', '`jumlah`', '`jumlah`', 5, 22, -1, FALSE, '`jumlah`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->jumlah->Sortable = TRUE; // Allow sort
		$this->jumlah->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
		$this->fields['jumlah'] = &$this->jumlah;

		// tanggal
		$this->tanggal = new DbField('laporan_kas', 'laporan_kas', 'x_tanggal', 'tanggal', '`tanggal`', CastDateFieldForLike("`tanggal`", 0, "DB"), 133, 10, 0, FALSE, '`tanggal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->tanggal->Sortable = TRUE; // Allow sort
		$this->tanggal->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
		$this->fields['tanggal'] = &$this->tanggal;

		// kode_penjualan
		$this->kode_penjualan = new DbField('laporan_kas', 'laporan_kas', 'x_kode_penjualan', 'kode_penjualan', '`kode_penjualan`', '`kode_penjualan`', 200, 50, -1, FALSE, '`kode_penjualan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->kode_penjualan->Sortable = TRUE; // Allow sort
		$this->fields['kode_penjualan'] = &$this->kode_penjualan;

		// id_mutasi_kas
		$this->id_mutasi_kas = new DbField('laporan_kas', 'laporan_kas', 'x_id_mutasi_kas', 'id_mutasi_kas', '`id_mutasi_kas`', '`id_mutasi_kas`', 3, 11, -1, FALSE, '`id_mutasi_kas`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id_mutasi_kas->Sortable = TRUE; // Allow sort
		$this->id_mutasi_kas->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['id_mutasi_kas'] = &$this->id_mutasi_kas;

		// saldo_awal
		$this->saldo_awal = new DbField('laporan_kas', 'laporan_kas', 'x_saldo_awal', 'saldo_awal', '`saldo_awal`', '`saldo_awal`', 5, 22, -1, FALSE, '`saldo_awal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->saldo_awal->Sortable = TRUE; // Allow sort
		$this->saldo_awal->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
		$this->fields['saldo_awal'] = &$this->saldo_awal;

		// sisa_saldo
		$this->sisa_saldo = new DbField('laporan_kas', 'laporan_kas', 'x_sisa_saldo', 'sisa_saldo', '`sisa_saldo`', '`sisa_saldo`', 5, 22, -1, FALSE, '`sisa_saldo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->sisa_saldo->Sortable = TRUE; // Allow sort
		$this->sisa_saldo->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
		$this->fields['sisa_saldo'] = &$this->sisa_saldo;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`laporan_kas`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->id_mutasi_kas->DbValue = $row['id_mutasi_kas'];
		$this->saldo_awal->DbValue = $row['saldo_awal'];
		$this->sisa_saldo->DbValue = $row['sisa_saldo'];
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "laporan_kasedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "laporan_kasadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("laporan_kasview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("laporan_kasview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "laporan_kasadd.php?" . $this->getUrlParm($parm);
		else
			$url = "laporan_kasadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("laporan_kasedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("laporan_kasadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("laporan_kasdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->id_mutasi_kas->setDbValue($rs->fields('id_mutasi_kas'));
		$this->saldo_awal->setDbValue($rs->fields('saldo_awal'));
		$this->sisa_saldo->setDbValue($rs->fields('sisa_saldo'));
	}
	...
	public function renderEditRow()
	{
		...
		$this->id_mutasi_kas->EditValue = $this->id_mutasi_kas->CurrentValue;
		...
		$this->sisa_saldo->EditValue = $this->sisa_saldo->CurrentValue;
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					...
					$doc->exportCaption($this->id_mutasi_kas);
					$doc->exportCaption($this->saldo_awal);
					$doc->exportCaption($this->sisa_saldo);
				} else {
					$doc->exportCaption($this->id);
					$doc->exportCaption($this->id_klinik);
					$doc->exportCaption($this->id_kas);
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						...
						$doc->exportField($this->id_mutasi_kas);
						$doc->exportField($this->saldo_awal);
						$doc->exportField($this->sisa_saldo);
					} else {
						$doc->exportField($this->id);
						$doc->exportField($this->id_klinik);
						$doc->exportField($this->id_kas);
