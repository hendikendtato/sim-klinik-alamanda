...
/**
 * Table class for m_kas
 */
class m_kas extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'm_kas';
		$this->TableName = 'm_kas';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`m_kas`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('m_kas', 'm_kas', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// id_klinik
		$this->id_klinik = new DbField('m_kas', 'm_kas', 'x_id_klinik', 'id_klinik', '`id_klinik`', '`id_klinik`', 3, 11, -1, FALSE, '`id_klinik`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// nama
		$this->nama = new DbField('m_kas', 'm_kas', 'x_nama', 'nama', '`nama`', '`nama`', 200, 50, -1, FALSE, '`nama`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nama->Sortable = TRUE; // Allow sort
		$this->fields['nama'] = &$this->nama;

		// saldo
		$this->saldo = new DbField('m_kas', 'm_kas', 'x_saldo', 'saldo', '`saldo`', '`saldo`', 5, 22, -1, FALSE, '`saldo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`m_kas`";
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "m_kasedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "m_kasadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_kasview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_kasview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "m_kasadd.php?" . $this->getUrlParm($parm);
		else
			$url = "m_kasadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_kasedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_kasadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("m_kasdelete.php", $this->getUrlParm());
	}
	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		$data_sql = ExecuteRow("SELECT * FROM m_kas
								JOIN m_klinik ON m_kas.id_klinik = m_klinik.id_klinik WHERE m_kas.id = '".$rsnew['id']."'");
		$data_array = ['params' => [
			'id_kas' => $data_sql['id'],
			'nama_kas' => $data_sql['nama'],
			'id_klinik' => [
