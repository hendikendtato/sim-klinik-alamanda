...
class stok extends DbTable
{
	...
	public function __construct()
	{
		...
		// id_stok
		$this->id_stok = new DbField('stok', 'stok', 'x_id_stok', 'id_stok', '`id_stok`', '`id_stok`', 19, 11, -1, FALSE, '`id_stok`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->id_stok->IsAutoIncrement = TRUE; // Autoincrement field
		$this->id_stok->IsPrimaryKey = TRUE; // Primary key field
		$this->id_stok->Sortable = TRUE; // Allow sort
		$this->id_stok->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->id_stok->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
		$this->id_stok->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['id_stok'] = &$this->id_stok;

		// id_barang
		$this->id_barang = new DbField('stok', 'stok', 'x_id_barang', 'id_barang', '`id_barang`', '`id_barang`', 19, 11, -1, FALSE, '`id_barang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// jumlah
		$this->jumlah = new DbField('stok', 'stok', 'x_jumlah', 'jumlah', '`jumlah`', '`jumlah`', 5, 11, -1, FALSE, '`jumlah`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->jumlah->Sortable = TRUE; // Allow sort
		$this->jumlah->DefaultErrorMessage = $Language->phrase("IncorrectFloat");
		$this->fields['jumlah'] = &$this->jumlah;

		// id_klinik
		$this->id_klinik = new DbField('stok', 'stok', 'x_id_klinik', 'id_klinik', '`id_klinik`', '`id_klinik`', 3, 11, -1, FALSE, '`id_klinik`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->id_stok->setDbValue($conn->insert_ID());
			$rs['id_stok'] = $this->id_stok->DbValue;
		}
		return $success;
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('id_stok', $rs))
				AddFilter($where, QuotedName('id_stok', $this->Dbid) . '=' . QuotedValue($rs['id_stok'], $this->id_stok->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->id_stok->DbValue = $row['id_stok'];
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('id_stok', $row) ? $row['id_stok'] : NULL;
		else
			$val = $this->id_stok->OldValue !== NULL ? $this->id_stok->OldValue : $this->id_stok->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@id_stok@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	{
		global $Language;
		if ($pageName == "stokview.php")
			return $Language->phrase("View");
		elseif ($pageName == "stokedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "stokadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("stokview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("stokview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "stokadd.php?" . $this->getUrlParm($parm);
		else
			$url = "stokadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("stokedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("stokadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("stokdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "id_stok:" . JsonEncode($this->id_stok->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm != "")
			$url .= $parm . "&";
		if ($this->id_stok->CurrentValue != NULL) {
			$url .= "id_stok=" . urlencode($this->id_stok->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("id_stok") !== NULL)
				$arKeys[] = Param("id_stok");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->id_stok->CurrentValue = $key;
			else
				$this->id_stok->OldValue = $key;
	...
	{
		$this->id_stok->setDbValue($rs->fields('id_stok'));
	...
	public function renderListRow()
	{
		...
		// id_stok
		$this->id_stok->LinkCustomAttributes = "";
		$this->id_stok->HrefValue = "";
		$this->id_stok->TooltipValue = "";
	...
	public function renderEditRow()
	{
		...
		// id_stok
		$this->id_stok->EditAttrs["class"] = "form-control";
		$this->id_stok->EditCustomAttributes = "";
		$this->id_stok->ViewCustomAttributes = "";
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					$doc->exportCaption($this->id_stok);
