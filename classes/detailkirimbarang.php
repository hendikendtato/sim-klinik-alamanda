...
/**
 * Table class for detailkirimbarang
 */
class detailkirimbarang extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'detailkirimbarang';
		$this->TableName = 'detailkirimbarang';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`detailkirimbarang`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('detailkirimbarang', 'detailkirimbarang', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// id_kirimbarang
		$this->id_kirimbarang = new DbField('detailkirimbarang', 'detailkirimbarang', 'x_id_kirimbarang', 'id_kirimbarang', '`id_kirimbarang`', '`id_kirimbarang`', 3, 11, -1, FALSE, '`id_kirimbarang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->id_kirimbarang->IsForeignKey = TRUE; // Foreign key field
		...
		// id_barang
		$this->id_barang = new DbField('detailkirimbarang', 'detailkirimbarang', 'x_id_barang', 'id_barang', '`id_barang`', '`id_barang`', 3, 11, -1, FALSE, '`id_barang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// harga
		$this->harga = new DbField('detailkirimbarang', 'detailkirimbarang', 'x_harga', 'harga', '`harga`', '`harga`', 5, 22, -1, FALSE, '`harga`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// id_satuan
		$this->id_satuan = new DbField('detailkirimbarang', 'detailkirimbarang', 'x_id_satuan', 'id_satuan', '`id_satuan`', '`id_satuan`', 3, 11, -1, FALSE, '`id_satuan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// jumlah
		$this->jumlah = new DbField('detailkirimbarang', 'detailkirimbarang', 'x_jumlah', 'jumlah', '`jumlah`', '`jumlah`', 4, 12, -1, FALSE, '`jumlah`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getMasterFilter()
	{
		...
		// Master filter
		$masterFilter = "";
		if ($this->getCurrentMasterTable() == "kirimbarang") {
			if ($this->id_kirimbarang->getSessionValue() != "")
				$masterFilter .= "`id`=" . QuotedValue($this->id_kirimbarang->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
		}
		return $masterFilter;
	...
	public function getDetailFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "kirimbarang") {
			if ($this->id_kirimbarang->getSessionValue() != "")
				$detailFilter .= "`id_kirimbarang`=" . QuotedValue($this->id_kirimbarang->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
		}
		return $detailFilter;
	...
	// Master filter
	public function sqlMasterFilter_kirimbarang()
	{
		return "`id`=@id@";
	}

	// Detail filter
	public function sqlDetailFilter_kirimbarang()
	{
		return "`id_kirimbarang`=@id_kirimbarang@";
	}

	// Table level SQL
	public function getSqlFrom() // From
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`detailkirimbarang`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "detailkirimbaranglist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "detailkirimbarangview.php")
			return $Language->phrase("View");
		elseif ($pageName == "detailkirimbarangedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "detailkirimbarangadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("detailkirimbarangview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("detailkirimbarangview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "detailkirimbarangadd.php?" . $this->getUrlParm($parm);
		else
			$url = "detailkirimbarangadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("detailkirimbarangedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("detailkirimbarangadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("detailkirimbarangdelete.php", $this->getUrlParm());
	}

	// Add master url
	public function addMasterUrl($url)
	{
		if ($this->getCurrentMasterTable() == "kirimbarang" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
			$url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_id=" . urlencode($this->id_kirimbarang->CurrentValue);
		}
		return $url;
	}
	...
	public function renderEditRow()
	{
		...
		if ($this->id_kirimbarang->getSessionValue() != "") {
			$this->id_kirimbarang->CurrentValue = $this->id_kirimbarang->getSessionValue();
			$this->id_kirimbarang->ViewValue = $this->id_kirimbarang->CurrentValue;
			$curVal = strval($this->id_kirimbarang->CurrentValue);
			if ($curVal != "") {
				$this->id_kirimbarang->ViewValue = $this->id_kirimbarang->lookupCacheOption($curVal);
	...
	function Row_Inserted($rsold, &$rsnew) {
			...
			if($status_kirim == 'dikirim'){
				if($tanggal < $tanggal_sekarang){
					$id_kartustok_sebelumnya = ExecuteScalar("SELECT id_kartustok FROM kartustok WHERE id_barang='$id_barang' AND id_klinik = '$id_supplier' AND tanggal <= '$tanggal' ORDER BY tanggal DESC, id_kartustok DESC LIMIT 1");
					if($id_kartustok_sebelumnya != NULL OR $id_kartustok_sebelumnya != FALSE){

						//insert into kartustok
						$stok_akhir = ExecuteScalar("SELECT stok_akhir FROM kartustok WHERE id_kartustok = '$id_kartustok_sebelumnya'");
						$set_stok_akhir = $stok_akhir - $qty;
						if($set_stok_akhir <= 0){
							Execute("INSERT INTO kartustok (id_barang, id_klinik, id_kirimbarang, tanggal, stok_awal, keluar_kirim, stok_akhir) VALUES ('$id_barang', '$id_supplier', '$pid_kirimbarang', '$tanggal', '$stok_akhir', '$qty', '0')");
						} else {
							Execute("INSERT INTO kartustok (id_barang, id_klinik, id_kirimbarang, tanggal, stok_awal, keluar_kirim, stok_akhir) VALUES ('$id_barang', '$id_supplier', '$pid_kirimbarang', '$tanggal', '$stok_akhir', '$qty', '$set_stok_akhir')");
						}

						//update data kartustok
						$data_kartustok = ExecuteRows("SELECT id_kartustok, stok_awal, stok_akhir FROM kartustok WHERE id_barang='$id_barang' AND id_klinik = '$id_supplier' AND tanggal > '$tanggal' ORDER BY tanggal ASC, id_kartustok ASC");	
						foreach($data_kartustok AS $kartustok) {
							$id = $kartustok['id_kartustok'];
							$stok_awal = $kartustok['stok_awal'] - $qty;
							$stok_akhir = $kartustok['stok_akhir'] - $qty;
							Execute("UPDATE kartustok SET stok_awal='$stok_awal', stok_akhir='$stok_akhir' WHERE id_kartustok = '$id'");
						}

						//update stok master barang
						$stok_master = ExecuteScalar("SELECT stok FROM m_hargajual WHERE id_barang='$id_barang' AND id_klinik = '$id_supplier'");
						$stok_master_baru = $stok_master - $qty;
						if($stok_master_baru <= 0){
							Execute("UPDATE m_hargajual SET stok = '0' WHERE id_barang='$id_barang' AND id_klinik = '$id_supplier'");
						} else {
							Execute("UPDATE m_hargajual SET stok = '$stok_master_baru' WHERE id_barang='$id_barang' AND id_klinik = '$id_supplier'");
						}
					} else {
						$id_kartustok_setelahnya = ExecuteScalar("SELECT id_kartustok FROM kartustok WHERE id_barang='$id_barang' AND id_klinik = '$id_supplier' AND tanggal > '$tanggal' ORDER BY tanggal ASC, id_kartustok ASC LIMIT 1");
						if($id_kartustok_setelahnya != NULL OR $id_kartustok_setelahnya != FALSE) {

							//insert into kartustok
							$stok_akhir = ExecuteScalar("SELECT stok_akhir FROM kartustok WHERE id_kartustok = '$id_kartustok_setelahnya'");
							$set_stok_akhir = $stok_akhir - $qty;
							if($set_stok_akhir <= 0){
								Execute("INSERT INTO kartustok (id_barang, id_klinik, id_kirimbarang, tanggal, stok_awal, keluar_kirim, stok_akhir) VALUES ('$id_barang', '$id_supplier', '$pid_kirimbarang', '$tanggal', '$stok_akhir', '$qty', '0')");
							} else {
							...
							//update data kartustok
							$data_kartustok = ExecuteRows("SELECT id_kartustok, stok_awal, stok_akhir FROM kartustok WHERE id_barang='$id_barang' AND id_klinik = '$id_supplier' AND tanggal > '$tanggal' ORDER BY tanggal ASC, id_kartustok ASC");
							if($data_kartustok != NULL OR $data_kartustok != FALSE) {
								$id = $kartustok['id_kartustok'];
								$stok_awal = $kartustok['stok_awal'] - $qty;
								$stok_akhir = $kartustok['stok_akhir'] - $qty;
								Execute("UPDATE kartustok SET stok_awal='$stok_awal', stok_akhir='$stok_akhir' WHERE id_kartustok = '$id'");
