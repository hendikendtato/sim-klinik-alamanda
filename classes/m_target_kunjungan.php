...
/**
 * Table class for m_target_kunjungan
 */
class m_target_kunjungan extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'm_target_kunjungan';
		$this->TableName = 'm_target_kunjungan';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`m_target_kunjungan`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id_target_kunjungan
		$this->id_target_kunjungan = new DbField('m_target_kunjungan', 'm_target_kunjungan', 'x_id_target_kunjungan', 'id_target_kunjungan', '`id_target_kunjungan`', '`id_target_kunjungan`', 3, 11, -1, FALSE, '`id_target_kunjungan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id_target_kunjungan->IsAutoIncrement = TRUE; // Autoincrement field
		$this->id_target_kunjungan->IsPrimaryKey = TRUE; // Primary key field
		$this->id_target_kunjungan->Sortable = TRUE; // Allow sort
		$this->id_target_kunjungan->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['id_target_kunjungan'] = &$this->id_target_kunjungan;

		// id_cabang
		$this->id_cabang = new DbField('m_target_kunjungan', 'm_target_kunjungan', 'x_id_cabang', 'id_cabang', '`id_cabang`', '`id_cabang`', 3, 11, -1, FALSE, '`id_cabang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// tgl_awal
		$this->tgl_awal = new DbField('m_target_kunjungan', 'm_target_kunjungan', 'x_tgl_awal', 'tgl_awal', '`tgl_awal`', CastDateFieldForLike("`tgl_awal`", 0, "DB"), 133, 10, 0, FALSE, '`tgl_awal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// tgl_akhir
		$this->tgl_akhir = new DbField('m_target_kunjungan', 'm_target_kunjungan', 'x_tgl_akhir', 'tgl_akhir', '`tgl_akhir`', CastDateFieldForLike("`tgl_akhir`", 0, "DB"), 133, 10, 0, FALSE, '`tgl_akhir`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// target
		$this->target = new DbField('m_target_kunjungan', 'm_target_kunjungan', 'x_target', 'target', '`target`', '`target`', 5, 22, -1, FALSE, '`target`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// created
		$this->created = new DbField('m_target_kunjungan', 'm_target_kunjungan', 'x_created', 'created', '`created`', '`created`', 200, 255, -1, FALSE, '`created`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->created->Sortable = TRUE; // Allow sort
		$this->fields['created'] = &$this->created;

		// updated
		$this->updated = new DbField('m_target_kunjungan', 'm_target_kunjungan', 'x_updated', 'updated', '`updated`', '`updated`', 200, 255, -1, FALSE, '`updated`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`m_target_kunjungan`";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->id_target_kunjungan->setDbValue($conn->insert_ID());
			$rs['id_target_kunjungan'] = $this->id_target_kunjungan->DbValue;
		}
		return $success;
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('id_target_kunjungan', $rs))
				AddFilter($where, QuotedName('id_target_kunjungan', $this->Dbid) . '=' . QuotedValue($rs['id_target_kunjungan'], $this->id_target_kunjungan->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->id_target_kunjungan->DbValue = $row['id_target_kunjungan'];
	...
	{
		return "`id_target_kunjungan` = @id_target_kunjungan@";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('id_target_kunjungan', $row) ? $row['id_target_kunjungan'] : NULL;
		else
			$val = $this->id_target_kunjungan->OldValue !== NULL ? $this->id_target_kunjungan->OldValue : $this->id_target_kunjungan->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@id_target_kunjungan@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "m_target_kunjunganlist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "m_target_kunjunganview.php")
			return $Language->phrase("View");
		elseif ($pageName == "m_target_kunjunganedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "m_target_kunjunganadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("m_target_kunjunganview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("m_target_kunjunganview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "m_target_kunjunganadd.php?" . $this->getUrlParm($parm);
		else
			$url = "m_target_kunjunganadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_target_kunjunganedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("m_target_kunjunganadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("m_target_kunjungandelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "id_target_kunjungan:" . JsonEncode($this->id_target_kunjungan->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm != "")
			$url .= $parm . "&";
		if ($this->id_target_kunjungan->CurrentValue != NULL) {
			$url .= "id_target_kunjungan=" . urlencode($this->id_target_kunjungan->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("id_target_kunjungan") !== NULL)
				$arKeys[] = Param("id_target_kunjungan");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->id_target_kunjungan->CurrentValue = $key;
			else
				$this->id_target_kunjungan->OldValue = $key;
	...
	{
		$this->id_target_kunjungan->setDbValue($rs->fields('id_target_kunjungan'));
	...
	public function renderListRow()
	{
		...
		// id_target_kunjungan
		$this->id_target_kunjungan->LinkCustomAttributes = "";
		$this->id_target_kunjungan->HrefValue = "";
		$this->id_target_kunjungan->TooltipValue = "";
	...
	public function renderEditRow()
	{
		...
		// id_target_kunjungan
		$this->id_target_kunjungan->EditAttrs["class"] = "form-control";
		$this->id_target_kunjungan->EditCustomAttributes = "";
		$this->id_target_kunjungan->EditValue = $this->id_target_kunjungan->CurrentValue;
		$this->id_target_kunjungan->ViewCustomAttributes = "";
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					$doc->exportCaption($this->id_target_kunjungan);
		...
		while (!$recordset->EOF && $recCnt < $stopRec) {
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						$doc->exportField($this->id_target_kunjungan);
