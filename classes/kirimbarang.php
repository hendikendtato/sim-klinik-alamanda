...
/**
 * Table class for kirimbarang
 */
class kirimbarang extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'kirimbarang';
		$this->TableName = 'kirimbarang';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`kirimbarang`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('kirimbarang', 'kirimbarang', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// no_kirimbarang
		$this->no_kirimbarang = new DbField('kirimbarang', 'kirimbarang', 'x_no_kirimbarang', 'no_kirimbarang', '`no_kirimbarang`', '`no_kirimbarang`', 200, 50, -1, FALSE, '`no_kirimbarang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->no_kirimbarang->Sortable = TRUE; // Allow sort
		$this->fields['no_kirimbarang'] = &$this->no_kirimbarang;

		// id_po
		$this->id_po = new DbField('kirimbarang', 'kirimbarang', 'x_id_po', 'id_po', '`id_po`', '`id_po`', 3, 11, -1, FALSE, '`id_po`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->id_po->Lookup = new Lookup('id_po', 'purchaseorder', FALSE, 'id_po', ["no_po","","",""], [], [], [], [], ["id_supplier","idklinik"], ["x_id_supplier","x_id_klinik"], '', '');
		$this->id_po->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['id_po'] = &$this->id_po;

		// id_supplier
		$this->id_supplier = new DbField('kirimbarang', 'kirimbarang', 'x_id_supplier', 'id_supplier', '`id_supplier`', '`id_supplier`', 3, 11, -1, FALSE, '`id_supplier`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// id_klinik
		$this->id_klinik = new DbField('kirimbarang', 'kirimbarang', 'x_id_klinik', 'id_klinik', '`id_klinik`', '`id_klinik`', 3, 11, -1, FALSE, '`id_klinik`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// id_pegawai
		$this->id_pegawai = new DbField('kirimbarang', 'kirimbarang', 'x_id_pegawai', 'id_pegawai', '`id_pegawai`', '`id_pegawai`', 3, 11, -1, FALSE, '`id_pegawai`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// tanggal
		$this->tanggal = new DbField('kirimbarang', 'kirimbarang', 'x_tanggal', 'tanggal', '`tanggal`', CastDateFieldForLike("`tanggal`", 7, "DB"), 135, 19, 7, FALSE, '`tanggal`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "detailkirimbarang") {
			$detailUrl = $GLOBALS["detailkirimbarang"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_id=" . urlencode($this->id->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "kirimbaranglist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`kirimbarang`";
	}
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->no_kirimbarang->DbValue = $row['no_kirimbarang'];
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "kirimbarangedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "kirimbarangadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("kirimbarangview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("kirimbarangview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "kirimbarangadd.php?" . $this->getUrlParm($parm);
		else
			$url = "kirimbarangadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("kirimbarangedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("kirimbarangedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("kirimbarangadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("kirimbarangadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("kirimbarangdelete.php", $this->getUrlParm());
	}
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->no_kirimbarang->setDbValue($rs->fields('no_kirimbarang'));
	...
	public function renderEditRow()
	{
		...
		// no_kirimbarang
		$this->no_kirimbarang->EditAttrs["class"] = "form-control";
		$this->no_kirimbarang->EditCustomAttributes = "";
		if (!$this->no_kirimbarang->Raw)
			$this->no_kirimbarang->CurrentValue = HtmlDecode($this->no_kirimbarang->CurrentValue);
		$this->no_kirimbarang->EditValue = $this->no_kirimbarang->CurrentValue;
		$this->no_kirimbarang->PlaceHolder = RemoveHtml($this->no_kirimbarang->caption());
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					$doc->exportCaption($this->no_kirimbarang);
		...
		while (!$recordset->EOF && $recCnt < $stopRec) {
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						$doc->exportField($this->no_kirimbarang);
	...
	function Row_Inserting($rsold, &$rsnew) {
		...
		$kode_kirim_sebelumnya = ExecuteScalar("SELECT no_kirimbarang FROM kirimbarang WHERE id_klinik=$id_klinik ORDER BY id DESC");
		 ...
		 *	0000 = nmr urut
		*/
		$rsnew['no_kirimbarang'] = 'KB' . $id_klinik . '-' . date('ym') . '-' . $nomor_urut;
		return TRUE;
	}
	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		//echo "Row Inserted"
		//ubah status_po menjadi 'kirim'

		$id_po = $rsnew["id_po"];
		Execute("UPDATE purchaseorder SET status_po = 'sent' WHERE id_po = '$id_po'");
	}
